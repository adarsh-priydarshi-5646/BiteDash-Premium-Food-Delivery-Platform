  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow mb-4">
      ...
      <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
        ...
        <span
          className={`px-3 py-1 rounded-md text-sm font-semibold border capitalize ${getStatusColor(currentStatus)}`}
        >
          {currentStatus}
        </span>
      </div>
      ...
      {showRatingModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl transform transition-all scale-100">
            ...
            <div className="mb-6 flex flex-col items-center">
              <div className="flex gap-2 mb-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    key={star}
                    className={`text-3xl transition-transform hover:scale-110 ${orderRating >= star ? 'text-yellow-400' : 'text-gray-200'}`}
              ...
              <p className="text-sm font-medium text-gray-600">
                {orderRating > 0 ? 'Thanks!' : 'Tap to rate'}
              </p>
            </div>
            <textarea
            ...
            <div className="flex gap-3">
              <button
                className="flex-1 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100"
                onClick={() => setShowRatingModal(false)}
              >
                Cancel
              </button>
              <button
                className="flex-1 bg-[#E23744] hover:bg-[#c02a35] text-white py-2 rounded-lg text-sm font-bold shadow-md"
