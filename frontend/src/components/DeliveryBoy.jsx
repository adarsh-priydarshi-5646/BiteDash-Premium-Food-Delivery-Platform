...
function DeliveryBoy() {
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      await axios.post(
        `${serverUrl}/api/order/cancel-order`,
        {
          orderId: currentOrder._id,
          shopOrderId: currentOrder.shopOrder._id,
          reason: finalReason,
        ...
        {!currentOrder && (
          <div className="w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="bg-[#E23744] text-white text-xs px-2 py-0.5 rounded-full">
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden scale-100 animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4">
                Please select a reason for cancellation. This will be logged.
              </p>
              ...
              <div className="space-y-3 mb-4">
                {cancelReasons.map((reason) => (
                  <label
                    key={reason}
                    className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${cancelReason === reason ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-red-200'}`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full border flex items-center justify-center ${cancelReason === reason ? 'border-red-500' : 'border-gray-300'}`}
                    >
                      {cancelReason === reason && (
                    ...
                    />
                    <span
                      className={`text-sm font-medium ${cancelReason === reason ? 'text-red-700' : 'text-gray-700'}`}
                    >
                      {reason}
                    </span>
                  </label>
