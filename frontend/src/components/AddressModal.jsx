    ...
    try {
      ...
      if (addressToEdit) {
        ...
        response = await axios.post(
          `${serverUrl}/api/user/add-address`,
          {
            address: formData,
          },
          { withCredentials: true }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      >
        ...
        <form
          ...
          className="p-6 space-y-6 max-h-[85vh] overflow-y-auto no-scrollbar"
        >
          <button
            type="button"
            ...
            className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 font-bold transition-all border relative overflow-hidden active:scale-95 disabled:opacity-70 ${
              detecting
                ? 'bg-red-100 border-red-200 text-[#E23744]'
                : 'bg-red-50 border-red-100 text-[#E23744] hover:bg-red-100'
            }`}
          >
            {detecting && (
              <motion.div
          ...
          <div className="grid grid-cols-3 gap-3">
            {labelOptions.map((option) => (
              ...
              >
                <span
                  className={`text-xl ${formData.label === option.id ? 'text-[#E23744]' : 'text-gray-400'}`}
                >
                  {option.icon}
                </span>
                <span
                  className={`text-xs font-bold ${formData.label === option.id ? 'text-[#E23744]' : 'text-gray-500'}`}
                >
                  {option.id}
                </span>
              </button>
